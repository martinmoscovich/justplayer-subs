...
| `app/src/main/java/com/brouken/player/PlayerActivity.java` | +1 import; +1 field `customSubtitles`; instantiate `CustomSubtitleController` after `setMediaItem` + `onMediaSet(...)`; release at both player-teardown points; +1 line in `dispatchKeyEvent` to route keys to the panel; +1 `setOnClickListener` on the subtitle button to open the panel. ~9 added lines, no existing logic changed (the subtitle button's default click — the Media3 track menu — is replaced; we manage subtitles). Later, unrelated: `launchedWithMediaIntent` field, set in `onCreate`/`onNewIntent` whenever a launch hands over a video via `ACTION_VIEW` with a URI; hides `buttonOpen` (and skips its onboarding tap-target) when true. | Hook the custom subtitle system with the smallest possible footprint. All behaviour lives in `com.brouken.player.subs.*`. The `launchedWithMediaIntent` addition: an external caller (Nuvio) already picked what to play — browsing to something else from inside the player doesn't match what that caller expects back. |
