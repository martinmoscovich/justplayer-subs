...
| `app/src/main/java/com/brouken/player/PlayerActivity.java` | +1 import; +1 field `customSubtitles`; instantiate `CustomSubtitleController` after `setMediaItem` + `onMediaSet(...)`; release at both player-teardown points; +1 line in `dispatchKeyEvent` to route keys to the sync panel; +1 `setOnClickListener` on the subtitle button to open the sync panel. ~9 added lines, no existing logic changed (the subtitle button's default click — the Media3 track menu — is replaced; we manage subtitles). | Hook the custom subtitle overlay + sync panel with the smallest possible footprint. All behaviour lives in `com.brouken.player.subs.*`. |
